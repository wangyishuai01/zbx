...
Drop function if exists function_cusbuyhistory;
CREATE FUNCTION function_cusbuyhistory(dstart TIMESTAMP, dend TIMESTAMP, dflag VARCHAR(10),buyType INT(1),userID INT(11))
...
BEGIN
	...
	WHILE dateBox <= dend DO 
		IF userID = 0 THEN
			SELECT CONCAT(resultMap,'|',DATE_FORMAT(dateBox,dformat),':',count(1)) INTO resultMap FROM cus_buy_history WHERE 
